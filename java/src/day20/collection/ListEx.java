...
public class ListEx {
	...
	public static void main(String[] args) {
		...
		//1부터 5까지 차례대로 추가
		for(int i = 1 ; i <= 5 ; i++) {
			list.add(i); //정수 i가 박싱으로 Integer 클래스의 객체가 된 후 list에 추가됨
		}
		//1을 더 추가
		list.add(1); //list는 중복을 허용하기 때문에 추가 가능
		System.out.println(list);
		//1을 제거
		list.remove((Integer)1);//첫번째 만나는 1을 제거하고 제거가 되어서 true를 반환 //1앞에 Integer를 붙여줘야함. 그렇지 않을 경우, int로 정의되어 1번지를 제거하게 됨.
		System.out.println(list);
		//1번지에 있는 객체를 제거(3을 제거)
		Integer num = list.remove(1); //1번지에 있는 요소를 제거하고 제거한 요소를 반환
		System.out.println(num);
		System.out.println(list);
		System.out.println(list.size());//저장된 객체 수를 출력. 저장 가능한 최대 객체수가 아님.
