...
public class ClassInheritanceEx {
	...
	public static void main(String[] args) {
		...
		System.out.println(phone1.getNum()); // private으로 사용 불가한 경우 @Data를 통해서 사용 가능(getNum)
		phone1.take();
...
class Phone{
	private String num = "010-1111-5555"; // 전화번호 
	                                      // private으로 사용 불가한 경우 @Data를 불러와서 사용 가능 
...
class SmartPhone extends Phone{
	...
	public SmartPhone() {
		//부모 클래스의 생성자가 호출
		//super를 이용하여 부모 클래스의 생성자를 직접 호출하지 않으면
		//기본 생성자인 super()가 자동으로 추가가 되고 호출이 됨.
		//이 때, 부모클래스의 기본 생성자가 없으면 에러가 발생
		//super(); // 위의 부모클래스에 생성자가 눈에 보이지 않기 때문에(있지만 눈에만 안보임) 아래와 같이 "" 로 넣어줌
		super("");
	}
	@Override
	public void call(String num) { //@Override일 때 리턴타입, 매개변수 수정안됨.
		super.call(num);
