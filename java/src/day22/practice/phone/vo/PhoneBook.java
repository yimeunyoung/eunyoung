...
public class PhoneBook {
	...
	public boolean insertPhone(String name, String number) {
		...
		//전화번호가 비어 있으면 => 전화번호 패턴에 맞지 않으면 저장 X, 맞으면 저장O
	...
	public boolean updateName(String name, String updateName) {
		...
		//전화번호부에 수정할 이름에 해당하는 전화번호가 어디에 있는지 화인
		...
		//list.get(index).setName(updateName); //위의 두줄을 한줄로 쓰면 이렇게 쓰면 됨
	...
	public boolean updateNumber(String name, String number) {
		...
		//전화번호 패턴이 맞으면 전화번호 객체를 생성했을 때 전화번호가 있고, 
		//패턴이 맞지 않으면 빈 문자열이 있다.
		//있으면 name과 number를 이용하여 전화번호 객체를 생성하는 대신 checkNumber를 이용
	...
	//전화번호 출력
	public void print(Predicate<PhoneNumber> p) { //람다식 적용
