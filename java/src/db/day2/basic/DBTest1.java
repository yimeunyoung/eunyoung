...
import java.sql.*; /* .*로 바꿔주면 전체에 적용되어 매번 import 안해줘도 됨*/
import java.util.*;

public class DBTest1 { // 콘솔에서 입력한 정보를 db에 옮기는 작업
	...
	public static void main(String[] args) {
		...
		try {
			PreparedStatement ps = con.prepareStatement(sql); //prepared statement : ?를 이용하여 처리하는 방식
			//회원 가입할 아이디와 비번을 입력
			System.out.print("ID : ");
			String id = sc.next();
			System.out.print("PW : ");
