/*
개인이 운영하는 쇼핑몰 사이트 구축을 위한 요구사항을 정리

- 판매자는 1명으로 고정. 판매자가 관리자이다.

- 제품은 대분류, 중분류를 선택해서 등록한다.
- 제품은 판매자가 제품명, 제품코드, 제품 상세 설명, 옵션 종류, 수량, 제품 이미지(썸네일 포함)를 선택해서 등록
- 제품 옵션의 수량이 없으면 품절이라고 출력하고 해당 옵션은 구매할 수 없다.

- 회원은 아이디, 비번, 이메일, 전화번호, 이름, 주소를 입력하여 회원가입을 한다.
- 회원가입 시 아이디는 중복체크를 하고, 전화번호를 통해 본인인증을 한다.
...
주문(주문번호, 총금액, 결제금액, 사용마일리지, 적립마일리지, 상태, 아이디(FK), 배송지번호(FK))
장바구니(장바구니번호(PK), 수량, 아이디(FK), 옵션번호(FK))
리뷰(리뷰번호(PK), 내용, 이미지, 아이디(FK), 옵션번호(FK))
주문목록(주문목록번호(FK), 수량, 총가격, 주문번호(FK), 옵션번호(FK))
*/