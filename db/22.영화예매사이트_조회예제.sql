...
WHERE 
	MO_OPENING_DATE <= NOW()
HAVING -- 집계 함수의 조건을 걸때 사용
	COUNT(MS_NUM) = 0; -- 집계 함수는 WHERE절에 넣을 수 없다. 

-- 영화 콘크리트 유토피아 리뷰를 조회하는 쿼리
SELECT * FROM REVIEW
WHERE RE_MO_NUM = (SELECT MO_NUM FROM MOVIE WHERE MO_TITLE = '콘크리트 유토피아');

-- 15세관람가 영화를 조회하는 쿼리
SELECT * FROM MOVIE WHERE MO_AG_NAME = '15세 관람가'; 

-- 이병헌이 출연한 영화를 조회하는 쿼리
SELECT MO_TITLE AS 영화, FP_NAME AS 영화인 
FROM 
	MOVIE
		JOIN 
	ROLE ON RO_MO_NUM = MO_NUM 
		JOIN 
	FILM_PERSON ON RO_FP_NUM = FP_NUM 
WHERE 
	FP_NAME = '이병헌';
    
-- 2023년에 개봉한 영화를 조회하는 쿼리
SELECT * FROM MOVIE WHERE MO_OPENING_DATE LIKE '2023%';

-- 2023년에 개봉한 한국 영화를 조회하는 쿼리
SELECT CP_CT_NAME AS 국가, MO_TITLE AS 영화 
FROM 
	MOVIE
		JOIN
	COUNTRY_PRODUCTION ON CP_MO_NUM = MO_NUM
WHERE MO_OPENING_DATE LIKE '2023%' AND CP_CT_NAME = '한국';

-- 각 영화의 리뷰수를 조회하는 쿼리 (선생님것과 비교하기!!!)
SELECT 
	MO_TITLE AS 영화명, ifnull(COUNT(RE_NUM),0) AS 리뷰수
FROM 
	REVIEW
		RIGHT JOIN
