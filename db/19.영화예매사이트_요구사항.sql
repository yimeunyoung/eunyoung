...
- 회원만 예매할 수 있다.
- 영화를 예매하기 위해서는 영화 선택, 영화관 선택, 스케쥴(시간대) 선택, 좌석을 선택(여러좌석)해서 예매
...
- 관리자는 영화를 선택하고, 상영종료를 클릭하여 해당 영화를 상영 종료할 수 있다.
- 상영 종료된 영화는 예매할 수 없다.

- 관리자는 영화관 등록 시, 지역을 선택하고, 영화관명, 주소, 전체 상영관수, 전체 좌석수, 각 상영관을 입력하여 등록한다.
- 각 상영관은 상영관명을 입력하고, 해당 상영관의 전체좌석수, 좌석들을 등록한다.
- 상영관에서 좌석 등록 시 행은 영어로, 열은 숫자로 표기하여 행과 열로된 좌석 이름, 좌석의 특징을 등록한다. 
...
- 회원가입 시, 아이디, 비번, 이름, 전화번호, 생년월일을 입력하고, 모든 항목은 필수 항목이다.
