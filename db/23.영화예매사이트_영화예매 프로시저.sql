...
BEGIN 
    ...
    */
	...
	-- 전체 요금을 계산 : 
    -- 성인수, 청소년수, 조조할인 여부를 알면 => 청소년 총 요금, 성인 총 요금을 계산
   ...
   OPEN CUR;
    READ_LOOP:LOOP
        ...
        -- 예약좌석에 가격번호를 연결하는데 있어서, 청소년 먼저하고, 성인을 하는것과
        -- 성인 먼저하고 청소년을 먼저하는 건 중요하지 않음
        -- 성인먼저 좌석 순으로 배치하고, 이후에 청소년을 좌석순으로 배치
		-- 성인수가 0이 아니면 주어진 좌석은 성인 좌석 그리고 0이면 청소년 좌석 배치
    ...
    UPDATE 
		MOVIE_SCHEDULE 
	SET _POSSIBLE_SEAT = (MS_POSSIBLE_SEAT =(
			SELECT 
